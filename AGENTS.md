...
- QVAC Whisper transcription exists via `createQvacTranscription`; structuring is heuristic via `createHeuristicStructuring`. Qwen3 is not wired. Active inference ports are `TranscriptionPort` and `StructuringPort`; the deleted `src/main/stt/` tree is not active. Storage includes the default in-memory note store and a JSON file store used when `notesFile` is passed. `saveNote` requires `clinicianConfirmed: true`, a generated draft, and source verification. Product vision + master prompt live in root `VISION.md`; visual system in `DESIGN.md`.
